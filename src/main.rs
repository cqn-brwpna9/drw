        ...
        match node.nodetype{
            ...
            ast::ASTnodeType::ControlStructure=>match node.structure.unwrap(){
                ast::ControlStructures::RepeatLoop=>{
                    ...
                    for _ in 0..n{
                        evallist(node.children.clone().unwrap(),data_stack,dip_stack,&mut turtle);
                    }
                },
            ...
            ast::ASTnodeType::Empty => todo!(), 
        }
    }
    return data_stack.to_string();
}

fn evallist(syntax_tree: Vec<ast::ASTnode>,
    data_stack: &mut stack::Stack<f64>,
    dip_stack: &mut stack::Stack<f64>,
    turtle: &mut Turtle,
){
        ...
        match node.nodetype{
            ...
            ast::ASTnodeType::ControlStructure=>match node.structure.unwrap(){
                ast::ControlStructures::RepeatLoop=>{
                    let n=data_stack.pop().unwrap() as i64;
                    for _ in 0..n{
                        evallist(node.children.clone().unwrap(),data_stack,dip_stack,turtle);
                    }
                },
                ast::ControlStructures::WhileLoop|
                ast::ControlStructures::IfBlock|
